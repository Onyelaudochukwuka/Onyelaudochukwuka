# 👋 Hi, I'm Udochukwuka Onyela (Udoka)

**Product‑centric Full Stack Engineer** · JavaScript · TypeScript · Python · React/Next.js · React Native · Node/Nest · Go · Rust

I design, build, and ship end‑to‑end products, from architecture and APIs to UX and growth, with a strong bias for measurable impact and reliability.

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Poppins&color=84A59D&size=30&lines=Product+Centric+Full+Stack+Engineer;Open+Sourcer;My+strong+points+are;Problem+Solving;Critical+Thinking;And+I+Love+Building+Stuff)](https://git.io/typing-svg)

---

## At a glance

- 5+ years of professional experience, 3+ years of daily coding and consistent open‑source activity  
- Lead / founding‑style engineer on early‑stage products and startups  
- Comfortable owning architecture, implementation, and product decisions end‑to‑end  

---

## Skills

**Languages & Core**

- JavaScript, TypeScript, Python  
- Go, Rust  
- HTML5, CSS3, SCSS (pre‑processors)  

**Frontend**

- React, Next.js  
- React Native  
- Tailwind CSS, Bootstrap  
- jQuery (legacy support)  

**Backend & APIs**

- Node.js, Express.js, Nest.js, FastApi
- REST, GraphQL  
- Testing with Jest  

**Tooling & Collaboration**

- Git/GitHub, CI/CD  
- Figma for design/product work
- Open Code
- Wakatime / metrics‑driven workflow  

---

## Selected projects

- **Lego‑build**  
  Flexible CLI that automates routine development tasks and makes local dev setup faster and less painful.

- **Status‑Server**  
  API that monitors website uptime and notifies users when saved sites go up or down.

- **Email‑backend**  
  TypeScript email backend focused on reliability, clear logging/observability, and production‑ready patterns.

- **Lendsqr FE Test**  
  Frontend for a lending‑as‑a‑service product with complex states, data tables, and real‑world product constraints.

(See pinned repositories on my profile for more details.)

---

## GitHub & activity

- 9,000+ contributions in the last year across personal projects, OSS, and startup work  
- Active member of multiple engineering communities and organizations  

You’ll find my GitHub stats, streaks, Wakatime graphs, and trophies below, they’re fun, but the main story is: I build a lot.

### Contribution Graph

![GitHub Activity Graph](https://github-readme-activity-graph.vercel.app/graph?username=Onyelaudochukwuka&theme=github-compact)


### Stats
![Udoka's GitHub stats](https://github-stats-seven-lemon.vercel.app/api?username=Onyelaudochukwuka&count_private=true)

### Coding Streaks
 ![GitHub Streak](https://github-readme-stats-fast.vercel.app/api/streak?username=Onyelaudochukwuka)

### Coding Time

[![wakatime](https://wakatime.com/badge/user/3cb90ed5-1bb9-4b97-a943-3c8e85a6dcbb.svg)](https://wakatime.com/@3cb90ed5-1bb9-4b97-a943-3c8e85a6dcbb)
<br />
[![Udoka's wakatime stats](https://github-stats-seven-lemon.vercel.app/api/wakatime?username=udoka)](https://github.com/anuraghazra/github-readme-stats)

### Github Trophies

[![trophy](https://github-trophy-nine.vercel.app/?username=Onyelaudochukwuka&title=Stars,Followers,MultiLanguage,Organizations,Commits,PullRequest,Issues,Stars,Repository)](https://github-trophy-nine.vercel.app/?username=Onyelaudochukwuka)


### Top Languages

[![Top Langs](https://github-stats-seven-lemon.vercel.app/api/top-langs/?username=Onyelaudochukwuka&count-private=true&langs_count=10)](https://github.com/Onyelaudochukwuka)
---

## Contact

- Website: [udoka.dev](https://www.udoka.dev)  
- X / Twitter: [@UdochukwukaO](https://x.com/UdochukwukaO)  
- Email: udochukwukaonyela@gmail.com  

Thanks for stopping by.
